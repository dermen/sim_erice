...
This simulator has the following interactive parameters:

Domain size along each edge: determines the size of the coherently scattering microcrystalline domains

Mosaic angle: extent of misorientation of domains relative to one another.

Brightness: overall image scaling (not a parameter of the experiment).

Display mode and update pinned image: a second copy of the simulation is held in memory ("pinned") and only updated upon request. When overlaying the current and pinned images in different color channels, a perfect match will render in grayscale, while pixel intensities affected by the difference will show up in color (cyan for the current simulation, red for the pinned one, and the rest of the spectrum inbetween). This can help emphasize what is changing as a parameter is adjusted and where on the detector this effect will be visible.

Reset all: regenerate the default simulation with all default parameters.

Unit cell scale: a scalar multiplier for each of the crystallographically independent unit cell axis lengths. (For crystal symmetries with codependent axis lengths, only a subset of scale factors are enabled.)

Rotation: tiny changes in the crystal orientation along the laboratory x, y and z axes, where z is the beam vector.

Randomize orientation: select a random orientation of the crystal in laboratory space (as we would experimentally observe in serial crystallography using a jet or similar sample delivery method). The U0 angles displayed will be updated to match.

Diffuse scattering: incorporate simulation of crystal imperfections. Gamma and sigma determine the diffuse scattering signal produced by long-range correlations. Gamma denotes the correlation length in Ångstroms and sigma squared is the amplitude of the correlated vibrations in square Ångstroms. Simulations with diffuse scattering enabled will be much quicker with monochromatic spectra.

Experiment mode: simulate a serial (stills) or a rotation experiment. In rotation mode, a series of simulations at different orientations across the requested oscillation width are summed to produce the final image. Multiple images in a continuous sweep can also be selected by image number.

Beam energy and bandwidth: where applicable (see spectrum shape), the mean energy and bandwidth of the pulse may be selected.

Spectrum shape: a monochromatic spectrum is simulated with a single energy channel and no bandwidth. This is most representative of a synchrotron experiment. A pink beam may be simulated with a Gaussian spectrum, which has 100 energy channels across the selected bandwidth. The SASE spectrum is the spiky, stochastic spectrum characteristic of a pulse generated by self-amplified spontaneous emission at an XFEL. It is drawn from a set of real recorded SASE spectra downsampled to 100 energy channels and shifted (as a series, not image-by-image) to the requested mean energy. A new pulse in the series can be selected with the "new XFEL pulse" button.

Use structure factors: sample the true molecular transform (on) or simulate a uniform intensity for all structure factors (off).

A deeper explanation of the parameters and their effects can be found at https://github.com/dermen/sim_erice#readme.
